...
VALUES ('testuser1', 'password1', 'ROLE_USER'),
       ('testuser2', 'password2', 'ROLE_USER');

INSERT INTO account (user_id, balance)
SELECT user_id, 1000.00
FROM tenmo_user;

-- Insert a test transfer
INSERT INTO transfer (transfer_type_id, transfer_status_id, account_from, account_to, amount)
VALUES (2, 2,
        (SELECT account_id FROM account WHERE user_id = (SELECT user_id FROM tenmo_user WHERE username = 'testuser1')),
        (SELECT account_id FROM account WHERE user_id = (SELECT user_id FROM tenmo_user WHERE username = 'testuser2')),
        500.00);
